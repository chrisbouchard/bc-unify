...
\usepackage[top=1in, bottom=1in, inner=1.5in, outer=1in, includefoot]{geometry}
...
\begin{titlepage}
    ...
    \vspace*{2in}
    \begin{center}
        \textbf{\huge Implementing Unification\\[+5pt] modulo Chaining} \\[+15pt]
...
We consider a rewrite system over a ranked signature and a possibly infinite
set of variables. We assume the reader is familiar with the usual notions and
concepts in term rewriting systems and equational unification~\cite{Term,
BaaderSnyd-01}. An \emph{equation} is an ordered pair of terms $(s, \, t)$,
usually written as $s \approx t$. Here $s$~is the
\emph{le\vspace{-0.2pt}ft-hand side} and $t$~is the \emph{right-hand side} of
the equation. A \emph{theory} is a set of equations, in which case the
...
rewrite rules. The signature of a theory $E$ will be denoted by $\Sigma_E$, and
the signature of a rewrite system $R$ by $\Sigma_R$.
...
\begin{enumerate}[(i)]
    ...
    \item $U \ueq a$, where $U$ is a variable and $a$ is a constant
    \item $U \ueq f(V_1, \dotsc, V_n)$, where $U$ and $V_1$ through $V_n$ are
        variables and $f$ is a function symbol
\end{enumerate}
...
$\Elt$ and $\List$, respectively.} Each function symbol in $\Sigma_\BC$ has an
...
equation $U \ueq \Cons(v, W)$ has sort $\List$, and we will refer to it as a
\emph{list equation}. Similarly, $u \ueq h(x, y)$ has sort $\Elt$ and is an
\emph{element equation}.
...
$\Hh$ should be semi-cancellative. A function symbol is
\emph{semi-cancel{}lative} iff it has the property that:
...
should be finitary. Finally, the signature of $\HH$ must be disjoint from
that of $\BC$ except for the symbol $\Hh$, i.e., $\Sigma_\BC \cap \Sigma_\HH
\subseteq \{h\}$.
...
$\Hh(x, y) \mapsto e_k(x \oplus y)$, where $\oplus$ is XOR and $e_k$ is an
...
strategy defined in \lstinline|INFERENCE| to apply the inference rules implemented
...
\begin{enumerate}[(L1), align=left, font=\bfseries]
    ...
    \item \emph{Cancel{}lation on $\Cons$}:
        \[\infer
            {\; \EQ ~ \cup ~ \{ U \ueq cons(v, W), \; x \ueq v, \; Y \ueq W \} \;}
            {\; \EQ ~ \uplus ~ \{ U \ueq cons(v, W), \; U \ueq cons(x, Y) \} \;}
        \]
...
\begin{enumerate}[(L4.a), align=left, font=\bfseries]
    \item \emph{Semi-Cancel{}lation on $\Bc$}, at a $\Bc/\Bc$-peak:
        \[\infer
...
version of the \lstinline|BC-| \lstinline|UNIFICATION| module instantiated with the view
...
An equations are given as a pair of terms separated by \lstinline|=?|
...
\end{lstlisting}
where the \lstinline|Qid| parameter is the name of the view. All terms
appearing in the unification problem must be valid meta-representations of
terms in the ``to'' module of the view. This includes, for example, element
constants, which must be explicitly declared. The module used in
\lstinline|TEST-BC-UNIFICATION| declares no element constants.
